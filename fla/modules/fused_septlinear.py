...
def weight_quant(w):
    ...
    quant_levels = torch.tensor([-2.0, -1.0, -0.5, 0.0, 0.5, 1.0, 2.0], device=w.device)
    ...
    # Compute the absolute differences between weights and quantization levels
    # Broadcast `scaled_w` against `quant_levels` to compare all pairs
    diff = (scaled_w.unsqueeze(-1) - quant_levels).abs()
    
    # Find the index of the nearest quantization level
    nearest_idx = diff.argmin(dim=-1)
    
    # Map the weights to the nearest quantization level
    quantized_w = quant_levels[nearest_idx]
...
class SeptLinear(nn.Linear):
    ...
    def __init__(self, in_features, out_features, bias=False):
        """
        Initializes the SeptLinear layer.
        ...
        super(SeptLinear, self).__init__(in_features, out_features, bias=bias)
...
class FusedSeptLinear(SeptLinear):
    """
    A custom linear layer that applies quantization on both activations and weights.
    This is primarily for training; kernel optimization is needed for efficiency in deployment.
    """
    ...
    def __init__(self, in_features, out_features, bias=False):
        ...
        """
        # Initialize the superclass nn.Linear with the given parameters
        super(FusedSeptLinear, self).__init__(in_features, out_features, bias=bias)
