...
class SeptNetPreTrainedModel(PreTrainedModel):

    config_class = SeptNetConfig
    base_model_prefix = 'model'
    supports_gradient_checkpointing = True
    _no_split_modules = ['SeptNetBlock']
    ...
    def _init_weights(
        ...
        if isinstance(module, (nn.Linear, SeptLinear, nn.Conv1d)):
...
class SeptNetModel(SeptNetPreTrainedModel):

    def __init__(
        self,
        config: SeptNetConfig
    ) -> SeptNetModel:
        ...
        self.layers = nn.ModuleList([SeptNetBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        **kwargs: Unpack[Any]
        ...
        if output_attentions:
            warnings.warn(
                "`SeptNetModel` does not support output attention weights now, so `output_attentions` is set to `False`."
            )
...
class SeptNetForCausalLM(SeptNetPreTrainedModel, GenerationMixin):
